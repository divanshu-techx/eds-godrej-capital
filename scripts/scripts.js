...
function calculateTabSectionCoordinate(
  main,
  lastTabBeginningIndex,
  targetTabSourceSection
) {
...
function calculateTabSectionCoordinates(main) {
  ...
  main.querySelectorAll("div.section[data-tab-title]").forEach((section) => {
    ...
    if (
      lastTabIndex < 0 ||
      currentSectionIndex - foldedTabsCounter !== lastTabIndex
    ) {
...
function aggregateTabSectionsIntoComponents(main) {
  ...
  Object.keys(tabElementMap).map(async (tabComponentIndex) => {
    ...
    await autoBlockTabComponent(
      main,
      tabComponentIndex - sectionIndexDelta,
      tabSections
    );
